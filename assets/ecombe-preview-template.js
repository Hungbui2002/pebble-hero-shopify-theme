...
window.LayoutHubSlider = (wrapper) => {
  ...
  function isAutoplayAllowed() {
    try {
      // Access read-only inspector state from parent window
      if (window.parent && window.parent !== window && window.parent.ebEditorState) {
        // In editor: disabledInspector === true means inspector is disabled, so autoplay is allowed
      ...
      // No ebEditorState found = live environment (published), allow autoplay
      return true;
    } catch (e) {
      // Cross-origin or access denied = likely live environment, allow autoplay
      return true;
    }
  }
  // =====================
  ...
  function setupAutoplay(node, autoplay, delay) {
    ...
    progressValue = 0;
    stopAtEnd = false;
    // Only allow autoplay if disabledInspector is true (inspector disabled = autoplay allowed)
    const autoplayAllowed = isAutoplayAllowed();
    if (autoplay && node && autoplayAllowed) {
      startAutoplay();
  ...
  function initSlider() {
    ...
    // Listen to inspector state changes from parent window
    function handleInspectorStateChange() {
      // Re-setup autoplay when inspector state changes
      const shouldAutoplay = emblaNode.getAttribute("data-play") === "true";
      setupAutoplay(emblaNode, shouldAutoplay, parseTransition(emblaNode.getAttribute("data-transition")));
    }
    
    // Add event listener for inspector state changes
    window.addEventListener("eb-inspector-state-change", handleInspectorStateChange);
...
})(document.querySelector('section.lh-7isb26'), 'live');
...
const videoWrapper = wrapper.querySelector('.eb-video-wrapper');
const coverImage = wrapper.querySelector('.eb-cover-image');
const usingCoverImage = videoWrapper.getAttribute('cover-image') === 'true';

if (usingCoverImage) {
  coverImage.addEventListener("click", () => {
    createVideo(coverImage, true);
  });
} else {
  createVideo(coverImage, true);
}

function createVideo(coverImage, shouldAutoplay = false) {

  if (coverImage.parentNode.querySelector('iframe, video')) {
    const existingClose = coverImage.parentNode.querySelector('.eb-close-button');
    if (existingClose) existingClose.style.display = '';
    coverImage.style.display = 'none';
    return;
  }

  const link = coverImage.getAttribute('link-video');
  const domain = getDomainFromUrl(link);
  const title = coverImage.getAttribute('alt-video') || '';
  const isLoop = coverImage.getAttribute('is-loop') === 'true';
  const height = coverImage.getBoundingClientRect().height || 600;
  let srcVideo, player;

  if (domain?.includes('youtube.com') || domain?.includes('youtu.be')) {
    const regExpYt = /^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)([^#&?]{11}).*/;
    const match = link.match(regExpYt);
    const idVideo = match && match[2] ? match[2] : null;
    if (!idVideo) return;
    const regexStartTime = /[?&]t=([0-9hms]+)/;
    const st = link.match(regexStartTime);
    const startTime = st ? st[1] : 0;
    const loop = isLoop ? `&loop=1&playlist=${idVideo}` : '';
    const autoplay = shouldAutoplay ? '&autoplay=1&mute=1' : '';
    srcVideo = `https://www.youtube.com/embed/${idVideo}?enablejsapi=1&start=${startTime}${loop}${autoplay}`;
  }

  else if (domain?.includes('vimeo.com')) {
    const regExpVimeo = /vimeo\.com\/(\d+)/;
    const match = link.match(regExpVimeo);
    const idVideo = match && match[1] ? match[1] : null;
    if (!idVideo) return;
    const autoplay = shouldAutoplay ? '&autoplay=1&muted=1' : '';
    srcVideo = `https://player.vimeo.com/video/${idVideo}?title=0&byline=0&portrait=0${autoplay}${isLoop ? '&loop=1' : ''}`;
  }
  ...
  if (link.match(/\.(mp4|webm|ogg)(\?.*)?$/i)) {
    ...
    coverImage.parentNode.insertBefore(player, coverImage);
    coverImage.style.display = 'none';

    handleCloseButton(coverImage, player);
    return;
  }
...
function handleCloseButton(coverImage) {
  let closeBtn = coverImage.parentNode.querySelector('.lh-close-button');

  if (!closeBtn) {
    closeBtn = createCloseButton(() => {
      const existing = coverImage.parentNode.querySelector('iframe, video');
      if (existing) existing.remove();

      closeBtn.style.display = 'none';
      coverImage.style.display = '';
    });
    coverImage.parentNode.appendChild(closeBtn);
    if(usingCoverImage) {
      const container = coverImage.closest('.eb-video-banner');
      if (container) {
        container.addEventListener("mouseenter", () => {
          const existing = container.querySelector('iframe, video');
          if(!existing) return;
...
function createCloseButton(onClick) {
  ...
  btn.innerText = '✖';
  btn.className = 'eb-close-button';
  btn.addEventListener('click', onClick);
  return btn;
}


})(document.querySelector('section.lh-mip5fm'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-qulhxp'), 'live');
}catch(ex){console.error(ex)}
...
const radios = wrapper.querySelectorAll('.lh-swatch-product-detail .lh-swatch input[type="radio"]');
...
const inventoryDefaultEl = wrapper.querySelector('.eb-product-info-wrapper .lh-product-inventory .inventory-default');
const lowStockEl = wrapper.querySelector('.eb-product-info-wrapper .lh-product-inventory .low-stock');
const inStockEl = wrapper.querySelector('.eb-product-info-wrapper .lh-product-inventory .in-stock');
...
function updateVariantUI(variant) {
  ...
  const priceBox = wrapper.querySelector('.eb-product-info-wrapper .lh-price-taxes');
  const priceEl = wrapper.querySelector('.eb-product-info-wrapper .lh-main-product-price .lh-special-price .lh-price');
  const compareEl = wrapper.querySelector('.eb-product-info-wrapper .lh-main-product-price .lh-old-price .lh-price');
  const addToCartBtn = wrapper.querySelector('.lh-product-button-add .btn-cart');
  const buyNowBtn = wrapper.querySelector('.lh-product-button-buy .btn_buy_now');
  const stock = wrapper.querySelector('.eb-product-info-wrapper .lh-product-inventory');
  ...
  const productBadgeContainer = wrapper.querySelector('.lh-main-product-badge');

  if (productBadgeContainer) {
    const isOnSale = variant.compare_at_price > variant.price && variant.price > 0;
    productBadgeContainer.querySelector(".lh-sale-badge")?.classList.toggle('tw-hidden', !isOnSale)
  }
...
function selectCallback(variant) {
  ...
  if (originalSlides === null) {
    originalSlides = Array.from(productMediaWrapper.querySelectorAll('.eb-product-media .lh-slide-item')).map(slide => ({
  ...
  if (originalThumbs === null) {
    originalThumbs = Array.from(productMediaWrapper.querySelectorAll('.eb-product-thumb-slide .lh-slide-item')).map(slide => ({
      element: slide.cloneNode(true),
      parent: slide.parentNode
    }));
  }
  ...
  originalSlides.forEach((slideData, index) => {
    ...
    const match = isFilterAlt === "true" ? normalizeText(altText).includes(normalizeText(colorOption)) : true;
    if (match) {
  ...
  if (isFilterAlt == "true" && slidesToShow.length === 0 && originalSlides.length > 0) {
  ...
  productMediaWrapper.querySelectorAll('.eb-product-media .lh-slide-item').forEach(slide => slide.remove());
  if (productMediaWrapper.emblaThumb) {
    productMediaWrapper.emblaThumb.slideNodes().forEach(thumb => thumb.remove());
  }

  const mainContainer = productMediaWrapper.querySelector('.eb-product-media .lh-slider-content');
  ...
  const thumbContainer = productMediaWrapper.querySelector('.eb-product-thumb-slide .lh-slider-content');
  ...
  if (slidesToShow.length > 0) {
    ...
    setTimeout(() => {
      const newSlides = wrapper.querySelectorAll('.eb-product-media .lh-slide-item');
      const newThumbs = wrapper.querySelectorAll('.eb-product-thumb-slide .lh-slide-item') || [];
      ...
      if (productMediaWrapper.emblaThumb) {
        ...
        const thumbWrapper = productMediaWrapper.querySelector('.eb-product-thumb-slide');
        if (thumbWrapper) {
...
setTimeout(() => {
  ...
  if (matchedVariant) {
    if (idInput && idInput.value !== String(matchedVariant.id)) {
      idInput.value = matchedVariant.id;
    }
    selectCallback(matchedVariant);
  } else {
    const firstValidVariant = productData.variants.find(v => v.available) || productData.variants[0];
    if (firstValidVariant) {
      if (idInput) idInput.value = firstValidVariant.id;
...
form?.addEventListener('submit', function (e) {
  ...
  if (!form || !quantityInput || !errorWrapper || !errorMessage) return;

  errorWrapper.style.display = 'none';

  let inputQty = parseInt(quantityInput.value, 10);
  ...
  const inputQtyForm = form.querySelector('input[name="quantity"]');
  if (inputQtyForm) {
    inputQtyForm.value = inputQty;
  }
...
const wrappers = document.querySelectorAll('.lh-product-description-wrapper');
wrappers.forEach(function (wrapper) {
  ...
  if (content && btn) {
    const checkVisibility = () => {
      if (wrapper.classList.contains('active') || content.scrollHeight > content.clientHeight) {
        ...
        btn.innerText = wrapper.classList.contains('active') ? textLess : textMore;
      } else {
...
const gallerySlides = Array.from(wrapper.querySelectorAll("#product-gallery .eb-product-media .lh-slide-item"));
...
function renderButtonZoom() {
  sliderWrapperEl.querySelectorAll('.lh-slide-item').forEach((item, index) => {
    ...
    button.className = 'lh-button-zoom eb-text-color-heading';
...
const getLightboxImages = (variant) => { 
  const productGallery = wrapper.querySelector("#product-gallery");
  const isFilterAlt = productGallery?.getAttribute("data-filter-alt") === "true";

  if (!gallerySlides || gallerySlides.length === 0) {
    // Return default placeholder images if gallery is empty
    return Array.from({ length: 5 }).map((_, i) =>
      `https://cdn.shopify.com/s/files/1/0326/1720/1797/files/image-collection-${i + 1}.svg?v=1757092840`
    );
  }

  const colorOption = normalizeText(variant?.options?.[0] || '');
  // Filter slides by matching alt text with variant option
  let filtered = gallerySlides
    .filter(slide => {
      if (!isFilterAlt) return true;
      const img = slide.querySelector('img');
      const altText = normalizeText(img?.alt || '');
      return altText.includes(colorOption);
    })
    .map(slide => slide.getAttribute("data-src"))
    .filter(Boolean);

  // If no matches, fallback to all slides
  if (!filtered.length) {
    filtered = gallerySlides
      .map(slide => slide.querySelector('img')?.getAttribute("data-src") || slide.getAttribute("data-src"))
      .filter(Boolean);
  }
...
const fillLightboxSlides = (container, images) => {
  ...
  images.forEach((src, i) => {
    ...
    slide.className = "lh-slide-item lh-grid-item";
    slide.setAttribute("data-src", src);
    slide.setAttribute("type", "image");
    slide.innerHTML = `
      <div class="lh-media-lb"> 
         <img src="${src}" class="lh-image" alt="Image ${i + 1}">
      </div>`;
    sliderContent.appendChild(slide);

    const thumb = document.createElement("div");
    thumb.className = "lh-thumb-item";
    thumb.innerHTML = `<img src="${src}" class="lh-image">`;
...
window.LayoutHubSlider = (wrapper) => {
  ...
  function initSlider() {
    ...
    function initEmblaThumbs(wrapper) {
      ...
      const emblaThumbNode = wrapper.querySelector('.eb-product-thumb-slide');
    ...
    // --- Mutation observer
    const mediaProductEl = wrapper.querySelector('.eb-media-product');
    if (mediaProductEl) {
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          if (
...
const productMediaWrapper = wrapper.querySelector('.eb-product-media-wrapper');
window.LayoutHubSlider(productMediaWrapper);


const sliderWrapperrEl = wrapper.querySelector('.eb-product-media');
...
function setupThumbClickHandlers() {
  const thumbs = wrapper.querySelectorAll('.eb-product-thumb-slide .lh-slide-item');
...
function updateThumbHeight() {
  const productMedia = wrapper.querySelector('.eb-product-media');
  const thumbSlide = wrapper.querySelector('.eb-product-thumb-slide');
...
resizeObserver.observe(wrapper.querySelector('.eb-product-media'));
...
const faqWrapper = wrapper.querySelector('.eb-faqs');
if (faqWrapper) {
  const items = faqWrapper.querySelectorAll('.eb-faq-content');

  function handleActive(item) {
    const openIcon = item.querySelector('.faq-icon-open');
    const closeIcon = item.querySelector('.faq-icon-close');
  ...
  items.forEach(item => {
    const btn = item.querySelector('.eb-item-question');
...
wrapper.querySelectorAll('.eb-rte').forEach(el => {
 el.innerHTML = el.textContent;
 ['h2', 'h3', 'h4', 'h5'].forEach(tag => {
   el.querySelectorAll(tag).forEach(h => {
     h.classList.add(`lh-${tag}`);
   });
 });
 const pClasses = ['lh-p', 'lh-p2', 'lh-p3'];
 const appliedClass = pClasses.find(c => el.classList.contains(c)) || 'lh-p';
 el.querySelectorAll('p').forEach(p => {
   p.classList.add(appliedClass);
 });
});
...
})(document.querySelector('section.lh-ivlpwx'), 'live');
}catch(ex){console.error(ex)}
...
const sliderContainer = wrapper.querySelector('.lh-slider-container');
const sliderContent = sliderContainer?.querySelector('.lh-slider-content');

function setDataLayout() {
  const slideItems = sliderContainer.querySelectorAll('.lh-slide-item');
  const countItem = slideItems.length;
  const itemDesktop = parseInt(sliderContainer.dataset.item, 10) || 0;

  const isLayoutSpecial = countItem - itemDesktop >= 2;

  sliderContainer.setAttribute('data-de-layout-special', isLayoutSpecial);
}
...
if (sliderContainer) {
  ...
  containerObserver.observe(sliderContainer, {
    attributes: true,
...
if (sliderContent) {
  ...
  contentObserver.observe(sliderContent, {
    childList: true,
    subtree: false
  });
}
...
const elements = wrapper.querySelectorAll('.eb-text-convert');
...
if (elements.length > 0) {
  ...
  function convertTextHTML(el) {
    const originalText = el.innerHTML;

    const processed = originalText
      .replace(/\[\[(.*?)\]\]/g, '<b>$1</b>')
      .replace(/&lt;b&gt;(.*?)&lt;\/b&gt;/g, '<b>$1</b>');
...
})(document.querySelector('section.lh-u7yup7'), 'live');
}catch(ex){console.error(ex)}

try {
((wrapper, mode) => {/* LayoutHub assets/script.js */
const titles = wrapper.querySelectorAll('.eb-item-question');
titles.forEach((title, index) => {
  title.addEventListener('click', () => {
    const parent = title.closest('.eb-item');
    if (parent.classList.contains('open-checkbox')) {
      parent.classList.remove('open-checkbox');
      parent.classList.toggle('active', index > 0);
    }
    else{
...
})(document.querySelector('section.lh-5n2cpy'), 'live');
}catch(ex){console.error(ex)}
 
    (function () {
      const cfg = window.__ECOMBE_PREVIEW__;
      if (!cfg || !cfg.enabled) return;
      if (window.__ECOMBE_PREVIEW_RENDERED__) return;

      window.__ECOMBE_PREVIEW_RENDERED__ = true;

      if (!document.body) return;

      document.body.insertAdjacentHTML("afterbegin", cfg.bannerHTML);
    })();
