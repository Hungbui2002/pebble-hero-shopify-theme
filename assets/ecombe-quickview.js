...
window.LayoutHubSlider = (wrapper) => {
  ...
  if (
    !container ||
    container.getAttribute("data-lh-slider-initialized") === "true"
  )
    return;
  ...
  function createPagination(wrapper, emblaNode, paginationType, totalPages) {
    ...
    if (paginationType === "dynamic-dots") {
      ...
      paginationWrapperInner.classList.add(
        "w-full",
  ...
  function initSlider() {
    ...
    function updateUI() {
      ...
      const paginationTotalEl = wrapper.querySelector(
        ".lh-slider-pagination-total span",
      );
      ...
      setTimeout(() => {
        if (controls) {
          ...
          if (width >= 1024 && totalItems > getItemsPerView(1024, emblaNode)) {
            ...
            width >= 640 &&
            width <= 1023 &&
            totalItems > getItemsPerView(640, emblaNode)
          ) {
    ...
    wrapper
      .querySelector(".lh-slider-nav-left")
      ?.addEventListener("click", embla.scrollPrev);
    wrapper
      .querySelector(".lh-slider-nav-right")
      ...
      const activatedObserver = new MutationObserver((mutationsList) => {
        for (const mutation of mutationsList) {
          if (
            ...
            if (
              activatedItem.getAttribute("data-lh-block-activated") === "true"
            ) {
  ...
  window.addEventListener("resize", () => {
    ...
    resizeTimeout = setTimeout(() => {
      ...
      if (Math.abs(currentWidth - lastWidth) >= 100) {
        let hasActivated = emblaNodeObserver.querySelector(
          "[data-lh-block-activated]",
        );
        if (hasActivated) {
          setTimeout(() => {
            let activeItem = emblaNodeObserver.querySelector(
              ".lh-slide-item.active",
            );
            if (activeItem) {
              activeItem.click();
            }
...
try {
  ((wrapper, mode) => {
    ...
    const productMediaWrapper = wrapper.querySelector(
      ".lh-product-media-wrapper",
    );
    window.LayoutHubSlider(productMediaWrapper);

    wrapper.querySelectorAll(".lh-collection-wrapper").forEach((el) => {
      window.LayoutHubSlider(el);
    });

    // function updatePaginationType() {
    //   const paginationType = window.innerWidth < 640 ? 'progress' : 'counter';
    //   document.querySelectorAll('.lh-collection-carousel .lh-slider-wrapper').forEach(wrapper => {
    //     wrapper.dataset.pagination = paginationType;
    //   });
    // }

    // updatePaginationType();
    // window.addEventListener('resize', updatePaginationType);
    ...
    const faqWrapper = wrapper.querySelector(".lh-faqs-content");
    if (faqWrapper) {
      const items = faqWrapper.querySelectorAll(".lh-faq-content");

      // const firstItem = items[0];
      // if (firstItem) {
      //   const answer = firstItem.querySelector('.lh-item-answer');
      //   const plusIcon = firstItem.querySelector('.faq-icon-plus');
      //   const minusIcon = firstItem.querySelector('.faq-icon-minus');
      ...
      items.forEach((item) => {
        const btn = item.querySelector(".lh-item-question");
        const answer = item.querySelector(".lh-item-answer");
        ...
        btn.addEventListener("click", () => {
          ...
          items.forEach((i) => {
            ...
            i.querySelector(".lh-item-answer")?.classList.add("tw-hidden");
            i.querySelector(".faq-icon-plus")?.classList.remove("tw-hidden");
            i.querySelector(".faq-icon-minus")?.classList.add("tw-hidden");
          });

          if (!isActive) {
            item.classList.add("active");
            answer.classList.remove("tw-hidden");
            if (plusIcon && minusIcon) {
    ...
    wrapper
      .querySelectorAll(".lh-btn-options[data-url]")
    ...
    const syncGridAndHotspot = () => {
      const targetSelector = ".lh-grid-item";
      const hotspotSelector = ".lh-dots-with-image .lh-hotspot";
      const dotSelector = ".lh-slider-pagination-wrapper button";

      const activateHotspotByIndex = (index) => {
        wrapper
          .querySelectorAll(hotspotSelector)
          .forEach((h) => h.classList.remove("active"));
        const matchedHotspot = wrapper.querySelector(
          `${hotspotSelector}[data-index="${index}"]`,
        );
        if (matchedHotspot) matchedHotspot.classList.add("active");
        markActiveItem();
      };

      const activateItemByIndex = (index) => {
        wrapper
          .querySelectorAll(targetSelector)
          .forEach((item) => item.classList.remove("active"));
        const matchedItem = wrapper.querySelector(
          `${targetSelector}[data-index="${index}"]`,
        );
        if (matchedItem) {
          matchedItem.classList.add("active");
          if (wrapper.embla) wrapper.embla.scrollTo(Number(index));
        }
        markActiveItem();
      };

      const markActiveItem = () => {
        wrapper
          .querySelectorAll(targetSelector)
          .forEach((item) => item.classList.remove("activeItem"));
        wrapper
          .querySelectorAll(hotspotSelector)
          .forEach((h) => h.classList.remove("active"));

        const totalItems = wrapper.querySelectorAll(targetSelector).length;
        const activeItems = Array.from(
          wrapper.querySelectorAll(`${targetSelector}.active`),
        );
        ...
        if (totalItems <= 2) {
          ...
          if (activeItems.length) {
            activeItems.forEach((item) => item.classList.add("activeItem"));
          }
          return;
        }

        if (activeItems.length === 1) {
          activeItems[0].classList.add("activeItem");

          const index = activeItems[0].dataset.index;
          const matchedHotspot = wrapper.querySelector(
            `${hotspotSelector}[data-index="${index}"]`,
          );
          if (matchedHotspot) matchedHotspot.classList.add("active");
        } else if (activeItems.length > 1) {
          const indexes = activeItems
          ...
          const chosenItem = activeItems.find(
            (el) => Number(el.dataset.index) === chosenIndex,
          );
          if (chosenItem) {
            chosenItem.classList.add("activeItem");

            const matchedHotspot = wrapper.querySelector(
              `${hotspotSelector}[data-index="${chosenIndex}"]`,
            );
            if (matchedHotspot) matchedHotspot.classList.add("active");
          }
        }
      ...
      const setupGridClick = () => {
        wrapper.querySelectorAll(targetSelector).forEach((item) => {
      ...
      const setupHotspotClick = () => {
        wrapper.querySelectorAll(hotspotSelector).forEach((hotspot) => {
      ...
      const setupDotClick = () => {
        wrapper.querySelectorAll(dotSelector).forEach((dot) => {
      ...
      const syncInitialState = () => {
        const activeItem = wrapper.querySelector(`${targetSelector}.active`);
        if (activeItem) {
          activateHotspotByIndex(activeItem.dataset.index);
        }
        markActiveItem();
      };

      if (wrapper.embla) {
        wrapper.embla.on("select", () => {
          markActiveItem();
        });
      }
    ...
    // =========================
    // 🎯 CUSTOM SELECT & VARIANT SYNC
    // =========================
    wrapper.querySelectorAll("form.lh-product-box").forEach((form) => {
      const productJsonEl = form.querySelector(".lh-product-json");
      const realSelect = form.querySelector(
        ".lh-single-option-selector[name='id']",
      );
      if (!productJsonEl || !realSelect) return;

      let productData;
      try {
        productData = JSON.parse(productJsonEl.textContent || "{}");
      } catch (err) {
        console.error("Parse product JSON failed:", err);
        return;
      }
      if (
      ...
      const selects = form.querySelectorAll(".lh-custom-select");
      const updateVariant = () => {
        const selectedValues = Array.from(selects).map((sel) => {
          const txt = sel.querySelector(".selected-text");
          return txt ? txt.textContent.trim().toLowerCase() : "";
        });

        let matchedVariant = productData.variants.find((v) => {
          const opts = v.options.map((o) => (o ? o.trim().toLowerCase() : ""));
          return opts.every(
            (optVal, i) => optVal === (selectedValues[i] || ""),
          );
        });

        if (!matchedVariant) matchedVariant = productData.variants[0];

        realSelect.value = matchedVariant.id;
        realSelect.dispatchEvent(new Event("change", { bubbles: true }));
        selects.forEach((sel, i) => {
          const txt = sel.querySelector(".selected-text");
          const opts = sel.querySelectorAll(".lh-option");
          const newVal = matchedVariant.options[i] || "";
          if (txt) txt.textContent = newVal;

          opts.forEach((optEl) => {
            const inputEl = optEl.querySelector("input");
            const optValue = optEl.dataset.value?.trim().toLowerCase() || "";
            if (inputEl)
              inputEl.checked = optValue === newVal.trim().toLowerCase();
          });
        });
      };

      wrapper
        .querySelectorAll(".lh-swatch-element input[type='radio']")
        .forEach((inputEl) => {
          inputEl.addEventListener("change", (e) => {
            const form = e.target.closest("form.lh-product-box");
            if (!form) return;

            const selects = form.querySelectorAll(".lh-custom-select");
            const optionIndex = e.target.name.replace("option-", "");
            const value = e.target.value;

            const select = Array.from(selects).find(
              (s) => s.dataset.optionIndex === optionIndex,
            );
            if (!select) return;

            const txt = select.querySelector(".selected-text");
            const preview = select.querySelector(".w-4.h-4.inline-block");
            const display = select.querySelector(".lh-select-display");

            if (txt) txt.textContent = value;
            if (display) display.setAttribute("aria-label", value);

            const colorKey = value.trim().toLowerCase().replace(/\s+/g, "-");
            if (preview) {
              const useImage = preview.style.backgroundImage.includes("url");
              if (useImage) {
                preview.style.backgroundImage = `var(--lh-swi-${colorKey})`;
              } else {
                preview.style.backgroundColor = `var(--lh-swc-${colorKey})`;
              }
            }

            const swatchWrapper = e.target.closest(".lh-custom-select");
            if (swatchWrapper) {
              const allSwatches =
                swatchWrapper.querySelectorAll(".lh-swatch-element");
              allSwatches.forEach((sw) => sw.classList.remove("active"));
              const parentSw = e.target.closest(".lh-swatch-element");
              if (parentSw) parentSw.classList.add("active");
            }
      ...
      selects.forEach((select) => {
        const display = select.querySelector(".lh-select-display");
        const optionsBox = select.querySelector(".lh-select-options");
        if (!display || !optionsBox) return;

        // display.addEventListener("click", (e) => {
        //   e.stopPropagation();

        //   wrapper.querySelectorAll(".lh-custom-select.active").forEach((other) => {
        //     other.classList.remove("active");
        //   });

        //   select.classList.add("active");

        //   const rect = optionsBox.getBoundingClientRect();
        //   const spaceBelow = window.innerHeight - rect.bottom;
        //   const spaceAbove = rect.top;
        //   if (spaceBelow < 100 && spaceAbove > spaceBelow) {
        //     optionsBox.classList.add("open-up");
        //   } else {
        //     optionsBox.classList.remove("open-up");
        //   }
        // });

        optionsBox.querySelectorAll(".lh-option").forEach((opt) => {
      ...
      //   wrapper.querySelectorAll(".lh-custom-select.active").forEach((sel) => {
      //     sel.classList.remove("active");
      //   });
      // });

      wrapper.addEventListener("click", (e) => {
        if (!e.target.closest(".lh-custom-select")) {
          wrapper
            .querySelectorAll(".lh-custom-select.active")
            .forEach((sel) => {
              sel.classList.remove("active");
              sel
                .querySelector(".lh-select-options")
    ...
    function updateActiveSwatchBackground(select) {
      const display = select.querySelector(".lh-select-display");
      const displayLabel = display?.getAttribute("aria-label");
      const swatches = select.querySelectorAll(".lh-swatch-element");
      ...
      swatches.forEach((sw) => {
        ...
        if (
          ...
          sw.style.background = "#dfdfdfab";
          sw.style.color = "#181818 !important";
    ...
    const selects = wrapper.querySelectorAll(".lh-custom-select");

    selects.forEach((select) => {
      const firstSwatch = select.querySelector(
        '.lh-swatch-element[data-index="0"]',
      );
      const firstInput = firstSwatch?.querySelector("input[type='radio']");
      const display = select.querySelector(".lh-select-display");

      if (firstSwatch && firstInput && display) {
        const firstLabel = firstInput.getAttribute("aria-label");
        display.setAttribute("aria-label", firstLabel);

        updateActiveSwatchBackground(select);
      }
    ...
    if (mode != "dev") {
      const bundleBtn = wrapper.querySelector("[data-product-bundle-submit]");
      const bundleMessage = wrapper.querySelector(".lh-error");
      ...
      const checkBundleAvailability = () => {
        ...
        wrapper.querySelectorAll("form.lh-product-box").forEach((form) => {
          ...
          if (!realSelect || !productJsonEl) return;
          ...
          const variant = productData.variants.find(
            (v) => v.id == Number(realSelect.value),
          );
          if (!variant || !variant.available || Number(variant.price) <= 0) {
            unavailable = true;
          }
        });
        ...
        if (unavailable) {
          ...
          bundleBtn.classList.add("opacity-50", "cursor-not-allowed");
          if (bundleMessage)
            bundleMessage.textContent =
              "One or more products are sold out or have a price of 0.";
        } else {
          bundleBtn.disabled = false;
          bundleBtn.classList.remove("opacity-50", "cursor-not-allowed");
          if (bundleMessage) bundleMessage.textContent = "";
        }
      };

      wrapper
        .querySelectorAll("form.lh-product-box .lh-single-option-selector")
        .forEach((select) => {
          select.addEventListener("change", checkBundleAvailability);
        });

      checkBundleAvailability();

      bundleBtn?.addEventListener("click", async (e) => {
        e.preventDefault();
        const btn = e.currentTarget;
        const spinner = btn.querySelector(".lh-loading__spinner");
        ...
        const forms = wrapper.querySelectorAll("form.lh-product-box");
        const itemsToAdd = [];
        ...
        try {
          ...
          if (bundleMessage)
            bundleMessage.textContent = "Failed to add bundle to cart.";
        }
      });
    }
    ...
    wrapper
      ...
      .forEach((input) => {
        input.addEventListener("change", function () {
          const swatchEl = this.closest(".lh-swatch-element");
          const newSrc = swatchEl.dataset.src;
          const productBox =
            this.closest(".lh-product-box") ||
            wrapper.querySelector(".lh-product-box");
          const mainImg = productBox.querySelector(".img-with-variant");
          const secondImg = productBox.querySelector(".img-second");
          ...
          if (newSrc && secondImg) {
            secondImg.setAttribute("src", newSrc);
            secondImg.setAttribute("srcset", newSrc);
          }
        });
      });

    const copyEls = wrapper.querySelectorAll(".lh-copy-url");

    copyEls.forEach((el) => {
      el.addEventListener("click", async () => {
        const link = el.dataset.copyTarget;
        const tooltip = el.querySelector(".lh-tooltip");
    ...
    const imgSelector =
      '.lh-product-media-wrapper[data-zoom="hover"] .lh-product-media .lh-slide-item .lh-image-picture';
    const zoomInCursor =
      'url(\'data:image/svg+xml;utf8,<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><circle opacity="0.8" cx="24" cy="24" r="24" fill="white"/><path d="M17 24H31M23.995 31L23.995 17" stroke="%231E1E1E" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/></svg>\') 12 12, zoom-in';
    const zoomOutCursor =
      'url(\'data:image/svg+xml;utf8,<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><circle opacity="0.8" cx="24" cy="24" r="24" fill="white"/><path d="M17 24H31" stroke="%231E1E1E" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/></svg>\') 12 12, zoom-out';

    function setCursor(type) {
      const cursor = type === "zoom-in" ? zoomInCursor : zoomOutCursor;
    ...
    const observers = new MutationObserver(() => {
      const overlay = document.querySelector(".media-hover-overlay");
      setCursor(overlay ? "zoom-out" : "zoom-in");
    });

    observers.observe(document.body, { childList: true, subtree: true });

    setCursor(
      document.querySelector(".media-hover-overlay") ? "zoom-out" : "zoom-in",
    );

    // const productGallerys = wrapper.querySelector('[data-position-thumbs]');
    // function updateThumbPosition() {
    //   if (!productGallerys) return;

    //   const width = window.innerWidth;

    //   if (width < 767) {
    //     productGallerys.setAttribute('data-position-thumbs', 'bottom');
    //   }
    // }
    // updateThumbPosition();
    ...
    wrapper.querySelectorAll("form.lh-product-box").forEach((form) => {
      ...
      const selectsCustom = form.querySelectorAll(".lh-custom-select");

      const updateUIForVariant = (variant) => {
        if (!variant) return;
        realSelect.value = variant.id;
        realSelect.dispatchEvent(new Event("change", { bubbles: true }));

        updateVariantPrices(form, variant);

        selectsCustom.forEach((sel, i) => {
          const txt = sel.querySelector(".selected-text");
          const opts = sel.querySelectorAll(".lh-option");
          const newVal = variant.options[i] || "";
          if (txt) txt.textContent = newVal;
      ...
      const updateVariantFromUI = () => {
        const selectedValues = Array.from(selectsCustom).map((sel) => {
          const txt = sel.querySelector(".selected-text");
          return txt ? txt.textContent.trim().toLowerCase() : "";
        });
        ...
        updateUIForVariant(matchedVariant);
      };
      ...
      wrapper
        ...
        .forEach((inputEl) => {
          inputEl.addEventListener("change", (e) => {
            ...
            const select = Array.from(selectsCustom).find(
              (s) => s.dataset.optionIndex === optionIndex,
            );
            if (!select) return;
    ...
    const selectsCustom = wrapper.querySelectorAll(".lh-custom-select");
    ...
    if (mode != "dev") {
      ...
      bundleBtn?.addEventListener("click", async (e) => {
        ...
        if (textSpan) textSpan.style.opacity = "0.4";
        spinner?.classList.remove("lh-hidden");
        if (bundleMessage) bundleMessage.textContent = "";
        ...
        if (itemsToAdd.length === 0) {
          spinner?.classList.add("lh-hidden");
          if (textSpan) textSpan.style.opacity = "1";
          return;
        }
    ...
    function reindexProductsAndLookbooks(wrapper) {
      const productItems = Array.from(
        wrapper.querySelectorAll(".lh-product-index"),
      ).filter(Boolean);
      const lookbookItems = Array.from(
        wrapper.querySelectorAll(".lh-lookbook-index"),
      ).filter(Boolean);
    ...
    reindexProductsAndLookbooks(wrapper);

    const observerTarget =
      wrapper.querySelector(".lh-slider-container") ||
      wrapper.querySelector(".lh-grid") ||
      wrapper;
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      for (const mutation of mutations) {
        if (
          ...
          [...mutation.addedNodes, ...mutation.removedNodes].some(
            (node) =>
              node.classList?.contains?.("lh-product-index") ||
              node.classList?.contains?.("lh-lookbook-index") ||
              node.querySelector?.(".lh-product-index") ||
              node.querySelector?.(".lh-lookbook-index"),
          )
        ) {
          shouldReindex = true;
          break;
      ...
      if (shouldReindex) {
        requestAnimationFrame(() => reindexProductsAndLookbooks(wrapper));
      }
    });
    ...
    function updateSwatchThumbnail(productBox, dataSrc) {
      const thumbnail = productBox.querySelector(
        ".lh-product-thumbnail .img-with-variant",
      );
    ...
    const bundleSubmitBtn = wrapper.querySelector(
      "[data-product-bundle-submit]",
    );
    if (mode !== "dev") {
      if (bundleSubmitBtn) {
        bundleSubmitBtn.addEventListener("click", () => {
          if (bundleState.items.length === 0) return;

          const itemsToAdd = bundleState.items.map((i) => ({
            id: i.variantId,
            quantity: i.quantity,
          }));
    ...
    function updateVariantPrices(form, variant) {
      const priceBox = form.querySelector(".lh-price-box");
      if (!priceBox) {
        console.warn("No .lh-price-box found inside form:", form);
        return;
      }

      const priceEl =
        priceBox.querySelector(".lh-price") ||
        priceBox.querySelector(".lh-current-price");
      const oldPriceEl = priceBox.querySelector(".lh-old-price");

      const settingsEl = document.querySelector("#money-settings");
      let rawFormat =
        settingsEl?.dataset.format ||
        window.Shopify?.money_format ||
        "{{amount}}";
      rawFormat = rawFormat.replace(/&quot;/g, '"').replace(/^"|"$/g, "");
      const moneyFormat = rawFormat;
      const currency =
        settingsEl?.dataset.currency || window.Shopify?.currency || "USD";
      const contactText =
        document.querySelector(".data-tex-locales")?.dataset.textContact ||
        "Contact";
      ...
      if (variant.price <= 0) {
        if (priceEl) priceEl.textContent = contactText;
        if (oldPriceEl) oldPriceEl.style.display = "none";
        priceBox.classList.remove("lh-on-sale");
        return;
      }
      const formattedPrice = formatMoney(variant.price, moneyFormat);
      const formattedCompare = variant.compare_at_price
      ...
      if (isSale) {
        priceBox.classList.add("lh-on-sale");

        if (oldPriceEl) {
          oldPriceEl.textContent = formattedCompare;
          oldPriceEl.style.display = "inline-flex";
        } else {
          const span = document.createElement("span");
          span.className =
            "lh-old-price h-full flex items-center line-through lh-p2 !leading-[20px] !text-scheme-text-500";
          span.innerHTML = `<span class="lh-p lh-price !text-2xl !leading-9 line-through !text-scheme-text-500">${formattedCompare}</span>`;
    ...
    /* LayoutHub assets/size_chart.js */
    ...
    if (button) {
      button.addEventListener("click", async (e) => {
        e.preventDefault();
        wrapper.querySelector("#lh-popup").classList.remove("tw-hidden");
    ...
    const closePopup = () => {
      wrapperPopup.classList.toggle("active");
      setTimeout(() => {
        wrapper.querySelector("#lh-popup").classList.add("tw-hidden");
    ...
    const gallerySlides = Array.from(
      wrapper.querySelectorAll(
        "#product-gallery .lh-product-media .lh-slide-item",
      ),
    );
    ...
    function renderButtonZoom() {
      sliderWrapperEl
        .querySelectorAll(".lh-slide-item")
        .forEach((item, index) => {
          const button = document.createElement("button");
          button.className =
            "lh-button-zoom absolute right-5 bottom-5 lh-text-color-heading";
          button.ariaLabel = "zoom";
          button.dataset.index = index;
          button.innerHTML = `
        <svg class="bi bi-zoom-in bg-scheme-bg w-10 h-10 p-3 rounded-[6px] " xmlns="http://www.w3.org/2000/svg" fill="currentColor"
          viewBox="0 0 16 16">
    ...
    const getLightboxImages = (variant) => {
      ...
      const isFilterAlt =
        productGallery?.getAttribute("data-filter-alt") === "true";

      let filtered = gallerySlides
        .filter((slide) => {
          const img = slide.querySelector("img");
          const altText = img?.alt?.toLowerCase() || "";
          const colorOption = variant?.options?.[0]?.toLowerCase() || "";

          // nếu có bật lọc theo alt, chỉ lấy ảnh có alt khớp với màu hiện tại
          return !isFilterAlt || altText.includes(colorOption);
        })
        .map((slide) => slide.getAttribute("data-src"))
        .filter(Boolean);

      // fallback nếu không có ảnh nào match
      if (!filtered.length) {
        filtered = Array.from({ length: 5 }).map(
          (_, i) =>
    ...
    <div class=" lh-carousel ">
      ...
      <div class="lh-slider-wrapper lh-slider lh-slider-container "
        ...
        data-play="false"
        style="--data-item: 1; --data-gap:0px;">
  ...
  `;
    ...
    const fillLightboxSlides = (container, images) => {
      ...
      const thumbnailsContent = container.querySelector(
        ".lh-thumbnails-content",
      );
      ...
      images.forEach((src, i) => {
        ...
        slide.className =
          "lh-slide-item lh-grid-item basis-full select-none tw-relative overflow-hidden";
        slide.setAttribute("data-src", src);
        slide.setAttribute("type", "image");
        slide.innerHTML = `
      <div class="lh-media-lb !tw-relative mx-auto mb-3 h-full tw-flex tw-items-center">
        ...
        thumb.addEventListener("click", () => {
          ...
          thumbnailsContent
            .querySelectorAll(".lh-thumb-item.active")
    ...
    boxLbContent.addEventListener("click", (e) => {
      ...
      const isNavButton = e.target.closest(
        ".lh-slider-nav-left, .lh-slider-nav-right",
      );
      ...
      if (
        e.target.closest(".lh-carousel") ||
        e.target.closest("#lh-lightbox")
      ) {
        closeLightbox();
      }
    });
    ...
    galleryContainer.addEventListener("click", (e) => {
      ...
      if (
        e.target.closest(".lh-slider-nav-right") ||
        e.target.closest(".lh-slider-nav-left")
      ) {
        resetZoomState();
      }
    });
    ...
    /* LayoutHub assets/product-form.js */
    const settingsEl = document.querySelector("#money-settings");
    ...
    function wrapperQueryAll(selector) {
      const all = wrapper.querySelectorAll(selector);
      return Array.from(all).filter(
        (el) => !el.closest(".lh-collection-wrapper"),
      );
    }

    function wrapperQuery(selector) {
      const el = wrapper.querySelector(selector);
      if (!el || el.closest(".lh-collection-wrapper")) return null;
      return el;
    }

    // ========== Setup ==========
    const productData = JSON.parse(
      wrapperQuery("#product-json-data")?.textContent || "{}",
    );
    const radios = wrapperQueryAll('.lh-swatch input[type="radio"]');
    const selectss = wrapperQueryAll(".single-option-selector");
    const form = wrapperQuery("#add-to-cart-form");
    const idInput = form?.querySelector('input[name="id"]');
    let currentVariant = null;
    const inventoryDefaultEl = wrapperQuery(
      ".details-pro .lh-product-inventory .inventory-default",
    );
    const lowStockEl = wrapperQuery(
      ".details-pro .lh-product-inventory .low-stock",
    );
    const inStockEl = wrapperQuery(
      ".details-pro .lh-product-inventory .in-stock",
    );
    const lowStockTemplate = lowStockEl?.innerHTML || "";
    const inStockTemplate = inStockEl?.innerHTML || "";
    const errorMessage = wrapperQuery(".lh-error-message");
    const maxMessage = errorMessage?.innerHTML || "";

    // ========== Quantity Controls (FINAL FIX) ==========
    const quantityContainers = wrapperQueryAll(".lh-product_quantity");
    quantityContainers.forEach((container) => {
      const input = container.querySelector(".qty-input");
      const btnIncrease = container.querySelector(".increase");
      const btnDecrease = container.querySelector(".reduced");

      let form = container.closest("form") || wrapperQuery("#add-to-cart-form");
      ...
      if (form) {
        ...
        const addToCartBtn = form.querySelector(
          '[type="submit"], .lh-add-to-cart, .btn-cart, [name="add"]',
        );
        if (addToCartBtn) {
          addToCartBtn.addEventListener("click", syncQuantity);
          addToCartBtn.addEventListener("mousedown", syncQuantity);
        }
      }
    });
    ...
    function syncSwatchInputs(variant) {
      variant.options.forEach((value, index) => {
        const swatch = wrapperQuery(`.lh-swatch[data-option-index="${index}"]`);
        if (swatch) {
    ...
    function updateQuantityErrorHandler() {
      const quantityInput = wrapperQuery(".qty-input");
      const errorWrapper = wrapperQuery(".lh-error-message-wrapper");
      ...
      if (!form.dataset.listenerAttached) {
        form.addEventListener("submit", function (e) {
          ...
          if (currentVariant?.available && inputQty > maxQty) {
            e.preventDefault();
            errorWrapper.style.display = "tw-flex";
    ...
    function updateVariantUI(variant) {
      ...
      const priceBox = wrapperQuery(".details-pro .lh-price-taxes");
      const realPriceBox = wrapperQuery(".details-pro .lh-price-box");

      const priceEl = wrapperQuery(".details-pro .lh-special-price .lh-price");
      const compareEl = wrapperQuery(".details-pro .lh-old-price .lh-price");
      const buttonActions = wrapperQuery(".button_actions");
      const addToCartBtn = buttonActions?.querySelector(".btn-cart");
      const buyNowBtn = buttonActions?.querySelector(".btn_buy_now");
      const stock = wrapperQuery(".details-pro .lh-product-inventory");
      const contactText =
        wrapperQuery(".data-tex-locales")?.dataset.textContact || "Contact";
      const outOfStockEl = stock?.querySelector(".out-of-stock");
      const skuEl = wrapperQuery(".variant-sku");

      const quantity = variant.inventory_quantity ?? 0;
      const stockThreshold =
        parseInt(stock?.getAttribute("data-stock-threshold")) || 20;
      ...
      if (variant.price > 0) {
        ...
        const isSale = variant.compare_at_price > variant.price;

        if (isSale) {
          compareEl &&
            ((compareEl.textContent = formatMoney(
              variant.compare_at_price,
              moneyFormat,
            )),
            (compareEl.style.display = "flex"));
          realPriceBox && realPriceBox.classList.add("lh-on-sale");
        } else {
          compareEl && (compareEl.style.display = "none");
          realPriceBox && realPriceBox.classList.remove("lh-on-sale");
        }
      } else {
      ...
      if (inventoryDefaultEl) inventoryDefaultEl.style.display = "none";

      if (quantity < stockThreshold) {
        if (lowStockEl) {
          lowStockEl.style.display = "flex";
          lowStockEl.innerHTML = lowStockTemplate.replace(
            "[quantity]",
            quantity,
          );
        }
    ...
    function updateVariantLabels(variant) {
      ...
      variant.options.forEach((optionValue, index) => {
        const swatch = wrapper.querySelector(
          `.lh-swatch[data-option-index="${index}"]`,
        );
        if (swatch) {
    ...
    function disableUnavailableSwatches() {
      ...
      wrapperQueryAll(".lh-swatch").forEach((swatchEl) => {
        const index = parseInt(swatchEl.dataset.optionIndex);
        const selectedInput = swatchEl.querySelector(
    ...
    let anyAltTextExists = false;
    const productGallery = wrapperQuery("#product-gallery");
    const isFilterAlt =
      productGallery?.getAttribute("data-filter-alt") === "true";
    ...
    function selectCallback(variant) {
      ...
      if (!originalSlides) {
        originalSlides = Array.from(
          wrapperQueryAll(".lh-product-media .lh-grid-item"),
        ).map((slide) => ({
          element: slide.cloneNode(true),
        }));
      }

      if (!originalThumbs && wrapper.emblaThumb) {
        originalThumbs = Array.from(wrapper.emblaThumb.slideNodes()).map(
          (thumb) => ({
      ...
      originalSlides.forEach((slideData, index) => {
        const img = slideData.element.querySelector("img");
        const altText = img?.getAttribute("alt")?.toLowerCase() || "";
        const thumbData = originalThumbs ? originalThumbs[index] : null;

        const include =
          !isFilterAlt || altText.includes(colorOption) || colorOption === "";

        if (include) {
          slidesToShow.push(slideData.element);
          if (thumbData) thumbsToShow.push(thumbData.element);

          if (variant.featured_image?.src) {
            const normalize = (url) => url?.replace(/^https?:/, "");
            const slideSrc = normalize(img?.getAttribute("src") || "");
            const featuredSrc = normalize(variant.featured_image.src);
            if (slideSrc === featuredSrc)
              activeSlideIndex = slidesToShow.length - 1;
          }
        }
      });
      ...
      if (
        ...
        originalSlides.forEach((slideData, index) => {
          ...
          if (variant.featured_image?.src) {
            ...
            const slideSrc = normalize(img?.getAttribute("src") || "");
            const featuredSrc = normalize(variant.featured_image.src);
            if (slideSrc === featuredSrc) activeSlideIndex = index; // Dùng 'index' vì đây là list đầy đủ
          }
        });

        if (activeSlideIndex >= slidesToShow.length) {
          activeSlideIndex = 0;
        }
      }
      wrapperQueryAll(".lh-product-media .lh-grid-item").forEach((slide) =>
        slide.remove(),
      );
      if (wrapper.emblaThumb)
        wrapper.emblaThumb.slideNodes().forEach((thumb) => thumb.remove());
      const mainContainer = wrapperQuery(
        ".lh-product-media .lh-slider-content",
      );
      slidesToShow.forEach((slide) => {
        slide.classList.remove("active", "active-prev", "active-next");
        slide.style.cssText = "";
        mainContainer.appendChild(slide);
      });

      if (wrapper.emblaThumb && thumbsToShow.length > 0) {
        const thumbContainer = wrapper.emblaThumb
          .rootNode()
          .querySelector(".lh-slider-content");
        thumbsToShow.forEach((thumb) => {
      ...
      if (slidesToShow.length > 0) {
        const targetIndex = Math.max(0, activeSlideIndex);
        setTimeout(() => {
          const newSlides = wrapperQueryAll(".lh-product-media .lh-grid-item");
          const newThumbs = wrapper.emblaThumb
            ? wrapper.emblaThumb.slideNodes()
            : [];

          newSlides[targetIndex]?.classList.add("active");
          newThumbs[targetIndex]?.classList.add("is-active");

          wrapper.embla?.reInit();
          wrapper.embla?.scrollTo(targetIndex);

          if (wrapper.emblaThumb) {
            wrapper.emblaThumb.reInit();
            wrapper.emblaThumb.scrollTo(targetIndex);

            wrapper.emblaThumb.slideNodes().forEach((thumb, index) => {
              thumb.addEventListener("click", () =>
                wrapper.embla?.scrollTo(index),
              );
            });

            const thumbWrapper = wrapperQuery(".lh-product-thumb-slide");
            if (thumbWrapper) {
              thumbWrapper.style.visibility = "visible";
              thumbWrapper.style.transition = "opacity 0.3s ease";
              requestAnimationFrame(() => (thumbWrapper.style.opacity = "1"));
            }
          }
        }, 50);
      }
    ...
    // ========== Bind swatch box click ==========
    const variantBoxes = wrapperQueryAll(".lh-variant-box");
    ...
    const stockInfoEl = wrapper.querySelector("#variant-stock-info");

    function updateStockMessage(variant) {
      if (!variant || !stockInfoEl) return;
      const stock = variant.inventory_quantity ?? 0;
      if (!variant.available) {
        stockInfoEl.innerHTML = `<span class="text-gray-500 mt-6">Out of stock</span>`;
      } else if (stock <= 5) {
        stockInfoEl.innerHTML = `<span class="lh-p2 left-in-stock ">Only ${stock} left in stock - Order soon</span>`;
      } else {
    ...
    radios.forEach((radio) => {
      radio.addEventListener("change", () => {
        const selectedOptions = [];
        wrapperQueryAll(".lh-swatch").forEach((sw, i) => {
    ...
    selectss.forEach((select) => {
      select.addEventListener("change", () => {
        const selectedOptions = [];
        selectss.forEach((sel) => {
    ...
    setTimeout(() => {
      const selectedOptions = [];
      wrapperQueryAll(".lh-swatch").forEach((swatchEl, i) => {
    ...
    // ========== Buy It Now ==========
    const buyNowBtn = wrapperQuery(".btn_buy_now");
    const quantityInput = wrapperQuery(".qty-input");
    ...
    wrapper
      .querySelectorAll(".lh-product-box[data-product-id]")
      .forEach((productBox) => {
        ...
        swatches.forEach((swatch) => {
          ...
          for (const radio of radios) {
            ...
            if (
              !radio.disabled &&
              !swatchEl?.classList.contains("lh-sold-out")
            ) {
              radio.checked = true;
              if (swatchEl?.classList.contains("lh-color")) {
                updateSwatchThumbnail(
                  productBox,
                  swatchEl.getAttribute("data-src"),
                );
              }
              break;
            }
        ...
        setTimeout(updateSelectedVariant, 10);
        ...
        function updateSelectedVariant() {
          const selectedOptions = Array.from(swatches).map(
            (swatch) =>
              swatch
          ...
          updateVariantPrice(productBox, matchedVariant);
          ...
          if (compareEl) {
            compareEl.innerHTML =
              ...
              matchedVariant.price > 0
                ? formatMoney(matchedVariant.compare_at_price, moneyFormat)
                : "";
          }
          ...
          if (button && idInput) {
            let productLabels = {};
            const labelScript = wrapper.querySelector("#product-button-labels");
            ...
            if (span) {
              if (!isAvailable) {
                span.textContent =
                  productLabels.inventoryOutOfStock || "Out of stock";
              } else if (isContact) {
                span.textContent =
                  productLabels.chooseOptions || "Choose options";
              } else {
    ...
    function updateVariantPrice(wrapper, variant) {
      const priceBox = wrapper.querySelector(".lh-price-box");
      if (!priceBox) return;

      const currentPriceEl =
        priceBox.querySelector(".lh-current-price") ||
        priceBox.querySelector(".lh-special-price .lh-price"); // Update selector dự phòng
      const oldPriceEl = priceBox.querySelector(".lh-old-price");
      ...
      if (currentPriceEl) currentPriceEl.textContent = formattedPrice;
      ...
      if (isSale) {
        ...
        if (oldPriceEl) {
          ...
          const span = document.createElement("span");
          span.className =
            "lh-old-price line-through !text-[var(--lh-other-sale-price-color)] lh-p2";
          span.textContent = formattedCompare;
          priceBox.appendChild(span);
        }
      } else {
        priceBox.classList.remove("lh-on-sale");
    ...
    style.innerHTML = `
  .lh-product-description-wrapper.hide-shadow .lh-desc-content::after {
...
`;
    ...
    const wrappers = document.querySelectorAll(
      ".lh-product-description-wrapper",
    );

    const generateLHId = () => {
      return "lh_" + Math.random().toString(36).substr(2, 6);
    };

    wrappers.forEach(function (wrapper) {
      const content = wrapper.querySelector(".lh-desc-content");
      const btn = wrapper.querySelector(".lh-toggle-desc-btn");
      ...
      if (content && btn) {
        const checkVisibility = () => {
          if (
            ...
            const textToUse = wrapper.classList.contains("active")
              ? textLess
              : textMore;

            let langTag = btn.querySelector("lh-lang");

            if (!langTag) {
              langTag = document.createElement("lh-lang");
              btn.innerHTML = "";
              btn.appendChild(langTag);

              const newId = generateLHId();
              langTag.setAttribute("data-lh-lang-id", newId);
            }
            langTag.innerText = textToUse;
            langTag.setAttribute("data-lh-lang-original", encodeURI(textToUse));
            // ------------------------------------------
          } else {
        ...
        if (content.scrollHeight > content.clientHeight) {
          const isFirstLoad = btn.getAttribute("data_first_load") === "true";
          if (isFirstLoad) {
  ...
  })(document.querySelector("section.lh-ery8tp"), "live");
} catch (ex) {
  console.error(ex);
}
