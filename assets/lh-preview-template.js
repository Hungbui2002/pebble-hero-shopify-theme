...
document.addEventListener("click", function (e) {
  const btn = e.target.closest(".coupon-code-btn");
  if (!btn) return;
  ...
  navigator.clipboard.writeText(code)
    ...
    .catch(() => {
      alert("複製失敗，請手動複製優惠碼: " + code);
    });
});


})(document.querySelector('section.lh-lugvmk'), 'live');
...
})(document.querySelector('section.lh-eboczp'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-6go5hh'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-k0jghy'), 'live');
}catch(ex){console.error(ex)}
