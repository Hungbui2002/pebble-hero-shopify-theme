...
const productAction = wrapper.querySelectorAll('.lh-product-action-btn');
productAction.forEach(p => {
...
wrapper.querySelectorAll('.lh-product-box[data-product-id]').forEach(productBox => {
  ...
  const isVariantSize = data.isVariantSize || false;
  const swatches = productBox.querySelectorAll('.lh-swatch');

  const buttons = isVariantSize ? productBox.querySelectorAll('.lh-btn-cart-with-size .lh-btn-cart') : null;
  ...
  function updateSelectedVariant() {
    ...
    const updatePriceAndCompare = (matchedVariant) => {
      ...
      if (compareEl) {
        const hasCompare = matchedVariant.compare_at_price > matchedVariant.price && matchedVariant.price > 0;
        compareEl.innerHTML = hasCompare
          ? formatMoney(matchedVariant.compare_at_price, moneyFormat)
          : '';
        compareContainer.classList.toggle('hidden', !hasCompare);
      }
    }

    const disableUnavailableSwatches = (selectedOptions) => {
      ...
      swatches.forEach((swatch, index) => {
        ...
        radios.forEach(radio => {
          ...
          const available = isVariantSize ? variants.some(v => v.options[0] === testOptions[0]) : variants.some(v =>
    ...
    if (isVariantSize) {
      const inputRadioSelected = Array.from(swatches).map(swatch =>
        swatch.querySelector('input[type="radio"]:checked')?.value.trim() || ''
      );

      buttons.forEach((button) => {
        const buttonValue = button.getAttribute('value');
        const selectedOptions = [...inputRadioSelected, buttonValue];
        ...
        button.addEventListener('click', () => {
          ...
          if (mode !== 'dev') productBox.submit();
        });

        updatePriceAndCompare(matchedVariant);

        disableUnavailableSwatches(selectedOptions);
      });
    }
    else {
...
})(document.querySelector('section.lh-hlt284'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-naeh0b'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-go3iaw'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-d7i7gi'), 'live');
}catch(ex){console.error(ex)}
...
try {
((wrapper, mode) => {})(document.querySelector('section.lh-2d4vay'), 'live');
}catch(ex){console.error(ex)}

try {
((wrapper, mode) => {})(document.querySelector('section.lh-lscw9e'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-vt3064'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-n2vsg7'), 'live');
}catch(ex){console.error(ex)}
