...
})(document.querySelector('section.lh-52ah5h'), 'live');
...
function setupMarquee(wrapperEl, contentEl) {
  const direction = wrapperEl.dataset.direction === 'right' ? 'right' : 'left';

  function setDirectionAnimation() {
    wrapperEl.style.animationName = direction === 'right' ? 'scroll-right' : 'scroll-left';
  }

  function duplicateItemsUntilFilled() {
    const allChildren = Array.from(contentEl.children);
    const originalItems = allChildren.filter(item => !item.dataset.cloned);
    if (originalItems.length === 0) return;

    allChildren.forEach(child => {
      if (child.dataset.cloned) {
        child.remove();
      }
    });
    ...
    let cloneWidth = originalItems.reduce((acc, el) => acc + el.offsetWidth, 0);
    let maxLoops = 20;

    while (totalWidth < requiredWidth && maxLoops > 0) {
  ...
  function cloneContent() {
    const last = wrapperEl.lastElementChild;
    if (last !== contentEl) last?.remove();
 ...
 const marqueeWrapper = wrapper.querySelector('.lh-marquee-wrapper');
const marqueeContent = marqueeWrapper?.querySelector('.lh-marquee-content');

let marquee;
if (marqueeWrapper && marqueeContent) {
  marquee = setupMarquee(marqueeWrapper, marqueeContent);
}

window.addEventListener('resize', debounce(() => {
  if (marquee) marquee.update();
}, 0));

window.dispatchEvent(new Event('resize'));


})(document.querySelector('section.lh-zof6ip'), 'live');
}catch(ex){console.error(ex)}

try {
((wrapper, mode) => {/* LayoutHub assets/script.js */

wrapper.querySelectorAll('.lh-tab-content').forEach(el => {
...
const btnOptions = wrapper.querySelectorAll('.lh-btn-options[data-url]');
btnOptions.forEach(btn => {
  ...
  wrapper.querySelectorAll(".lh-rating-preview").forEach(el => el.remove());
} else {
  wrapper.querySelectorAll(".lh-btn-cart").forEach(btn => {
    btn.style.pointerEvents = 'none';
  });
  wrapper.querySelectorAll(".lh-rating-live").forEach(el => el.remove());
}


function initTabs() {
  const tabs = wrapper.querySelectorAll('.lh-tab');
  const contents = wrapper.querySelectorAll('.lh-tab-content');
  const viewAllEl = wrapper.querySelector('.lh-btn-view-all');

  if (tabs && tabs.length > 0) {
    const updateViewAll = (tab) => {
      if (!viewAllEl) return;

      const isShowViewAll = tab.dataset.isShowView === 'true';
      const link = tab.dataset.url || '#';

      viewAllEl.style.display = isShowViewAll ? '' : 'none';
      viewAllEl.href = isShowViewAll ? link : '#';
    }
    ...
    tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        const target = wrapper.querySelector(`.tab-content-${index}`);
...
wrapper.querySelectorAll('.lh-product-box[data-product-id]').forEach(productBox => {
  ...
  setTimeout(updateSelectedVariant, 20);

});


})(document.querySelector('section.lh-nm5g22'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-p4oi0d'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-nj865b'), 'live');
}catch(ex){console.error(ex)}
...
const syncGridAndHotspot = () => {
  ...
  const activateHotspotByIndex = (index) => {
    wrapper.querySelectorAll(hotspotSelector).forEach(h => h.classList.remove('active'));
    const matchedHotspot = wrapper.querySelector(`${hotspotSelector}[data-index="${index}"]`);
    if (matchedHotspot) matchedHotspot.classList.add('active');
    markActiveItem();
  };

  const activateItemByIndex = (index) => {
    wrapper.querySelectorAll(targetSelector).forEach(item => item.classList.remove('active'));
    const matchedItem = wrapper.querySelector(`${targetSelector}[data-index="${index}"]`);
    if (matchedItem) {
  ...
  const markActiveItem = () => {
    wrapper.querySelectorAll(targetSelector).forEach(item => item.classList.remove('activeItem'));
    wrapper.querySelectorAll(hotspotSelector).forEach(h => h.classList.remove('active'));

    const totalItems = wrapper.querySelectorAll(targetSelector).length;
    const activeItems = Array.from(wrapper.querySelectorAll(`${targetSelector}.active`));
    ...
    if (activeItems.length === 1) {
      ...
      const indexes = activeItems.map(el => Number(el.dataset.index)).sort((a, b) => a - b);
      ...
      const chosenItem = activeItems.find(el => Number(el.dataset.index) === chosenIndex);
      if (chosenItem) {
        chosenItem.classList.add('activeItem');

        const matchedHotspot = wrapper.querySelector(`${hotspotSelector}[data-index="${chosenIndex}"]`);
        if (matchedHotspot) matchedHotspot.classList.add('active');
      }
    }
  };
...
function handleTranslateControlNavWithThumb (){
  const heightSliderWrapper = wrapper.querySelector('.lh-slider-wrapper').offsetHeight;
  const heightThumbSlider = wrapper.querySelector('.lh-product-thumbnail').offsetHeight;

  const progressHeight = wrapper.querySelector('.lh-slider-controls').offsetHeight;

  const progressAndContentHeight = heightSliderWrapper - heightThumbSlider + progressHeight;

  wrapper.style.setProperty('--translate-controls-nav', `${progressAndContentHeight}px`);
}

setTimeout(handleTranslateControlNavWithThumb,100);

const obsever = new MutationObserver(() => {
  handleTranslateControlNavWithThumb();
})

obsever.observe(wrapper.querySelector('.lh-slider-wrapper'), {
  attributeFilter: ['data-pagination'],
  attributes: true,
  subtree: false
})
...
})(document.querySelector('section.lh-qd5afc'), 'live');
}catch(ex){console.error(ex)}
...
try {
((wrapper, mode) => {/* LayoutHub assets/script.js */
window.runCountdown = function (wrapper) {
    const countdownWrp = wrapper.querySelector('.lh-countdown-wrapper');
    if (!countdownWrp) return;

    const typeStart = countdownWrp.getAttribute('data-type-start');
    const showWeek = countdownWrp.getAttribute('show-week') === 'true';

    const existingTimerId = wrapper.dataset.timerId;
    if (existingTimerId) clearInterval(Number(existingTimerId));

    const hours = parseInt(countdownWrp.getAttribute('hours'), 10) || 0;
    const minutes = parseInt(countdownWrp.getAttribute('minutes'), 10) || 0;
    let durationMs = (hours * 60 + minutes) * 60 * 1000;

    let startTime, endTime, storageKey;

    if (typeStart === 'first_visit') {
        const prefix = 'lh_countdown_key_';
        ...
        const startAttr = countdownWrp.getAttribute('start-time');
        const endAttr = countdownWrp.getAttribute('end-time');
    ...
    function updateCountdown() {
        ...
        const time = {
            ...
            day: showWeek
                ? Math.floor((totalSeconds % (7 * 24 * 3600)) / (24 * 3600))
        ...
        Object.entries(time).forEach(([key, value]) => {
            const el = wrapper.querySelector(`.lh-countdown-${key} > .lh-countdown-number`);
            if (el) el.textContent = String(value).padStart(2, '0');
        });

        if (remainingTime === 0) {
            const countdownAction = countdownWrp.getAttribute('countdown-action-time-out');
            clearInterval(timer);
            countdownWrp.classList.add('time-out');
            ...
            if (countdownAction === 'refresh') {
                if (typeStart !== 'first_visit' && typeStart !== 'default') {
                    ...
                    countdownWrp.setAttribute('start-time', newStartTime.toLocaleString());
                    countdownWrp.setAttribute('end-time', newEndTime.toLocaleString());
                    countdownWrp.classList.remove('time-out');
...
const bgImg = wrapper.querySelector(".lh-image-bg img");

if (bgImg) {
  const parent = bgImg.closest(".lh-image-content");
  const checkImg = () => {
    parent.style.display = bgImg.src.includes("data:image/svg") ? "none" : "";
  };
  checkImg();
  const observer = new MutationObserver(checkImg);
  observer.observe(bgImg, { attributes: true, attributeFilter: ["src", "srcset"] });
  bgImg.addEventListener("load", checkImg);
}


})(document.querySelector('section.lh-u9oi4l'), 'live');
}catch(ex){console.error(ex)}

try {
((wrapper, mode) => {/* LayoutHub assets/size_chart.js */
  ...
  lightbox.innerHTML = `
    <div class=" lh-carousel ">
      ...
      <div class="lh-slider-wrapper lh-slider lh-slider-container "
        ...
        data-play="false"
        style="--data-item: 2; --data-gap:0px;">
...
const selects = wrapper.querySelectorAll('.single-option-selector');
...
function updateVariantUI(variant) {
  ...
  const priceEl = wrapper.querySelector('.details-pro .lh-special-price .lh-price');
  const compareEl = wrapper.querySelector('.details-pro .lh-old-price .lh-price');
  ...
  if (!variant.available) {
    ...
    outOfStockEl && (outOfStockEl.style.display = 'inline');
    return;
  }
...
function disableUnavailableSwatches() {
  ...
  wrapper.querySelectorAll('.lh-swatch').forEach((swatchEl) => {
...
function selectCallback(variant) {
  ...
  if (originalSlides === null) {
    originalSlides = Array.from(wrapper.querySelectorAll('.lh-product-media .lh-grid-item')).map(slide => ({
      element: slide.cloneNode(true),
      parent: slide.parentNode
    }));
  }

  if (originalThumbs === null && wrapper.emblaThumb) {
    originalThumbs = Array.from(wrapper.emblaThumb.slideNodes()).map(thumb => ({
  ...
  originalSlides.forEach((slideData, index) => {
    ...
    if (isFilterAlt === "true") {
      const match = altText.toLowerCase().includes(colorOption.toLowerCase())
      if (match) {
  ...
  wrapper.querySelectorAll('.lh-product-media .lh-grid-item').forEach(slide => slide.remove());
  if (wrapper.emblaThumb) {
    wrapper.emblaThumb.slideNodes().forEach(thumb => thumb.remove());
  }

  const mainContainer = wrapper.querySelector('.lh-product-media .lh-slider-content');
  ...
  if (wrapper.emblaThumb && thumbsToShow.length > 0) {
    const thumbContainer = wrapper.emblaThumb.rootNode().querySelector('.lh-slider-content');
  ...
  if (slidesToShow.length > 0) {
    ...
    setTimeout(() => {
      ...
      if (wrapper.emblaThumb) {
        ...
        const thumbWrapper = wrapper.querySelector('.lh-product-thumb-slide');
        if (thumbWrapper) {
...
const variantBoxes = wrapper.querySelectorAll('.lh-variant-box');
...
})(document.querySelector('section.lh-vcdtqa'), 'live');
}catch(ex){console.error(ex)}

try {
((wrapper, mode) => {})(document.querySelector('section.lh-znzjmh'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-vuvjok'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-hmvyf3'), 'live');
}catch(ex){console.error(ex)}
...
})(document.querySelector('section.lh-qzq9ox'), 'live');
}catch(ex){console.error(ex)}
